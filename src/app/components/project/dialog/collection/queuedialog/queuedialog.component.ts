...
export class QueuedialogComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    this.jobUuid = this.uploadService.job.job.job_uuid;
    this.time = this.uploadService.job.estimate_time;
    this.position = this.uploadService.job.position;
    console.log('job uuid', this.jobUuid);
    this.startPoll();
  }
